...
function trackAJAXimpressions(productString)
{
	if(!(typeof s_account === "undefined"))
	{
		...
		s.tl(true,'o',s.pagename + 'AJAXImp');
		if(s.templtv) s.linkTrackVars=s.templtv;
		if(s.templte) s.linkTrackEvents=s.templte;
	}
}
function trackClickWithImpressions(qLinkName)
{
	if(!(typeof s_account === "undefined"))
	{
		...
		s.templtv=s.linkTrackVars;
		s.templte=s.linkTrackEvents;
		s.linkTrackVars='eVar6,products';
		s.eVar6=qLinkName;
...
s.linkInternalFilters="javascript:,interprise,uswest,lci,ridethelight,cybersolutions,northwesternbell,litel,qwestinteractive.com,highway,qintra.com,qwest.connectutilities.com,interact.kefta.com,qwest.groovemobile.net,qwest.com,qwestapps.com,techzillabyqwest.com,qwestaffiliateoffers.com,qwestcampaigns.com,qwestbestoffers.com,insideconnection.com,incredibleinternet.com,qwestwifilocator.com"
...
function s_doPlugins(s) {
	...
	if(!s.eVar6)
	{
		s.eVar6=s.getQueryParam('iq_id')
		if(s.eVar6){if(s.events)s.events=s.events+",event23"
		else s.events="event23"}
	//s.eVar6=s.getValOnce(s.eVar6,'s_eVar6',0);
}
	
	if(s.linkTrackVars.match("eVar6"))
	{
		s.linkTrackVars=s.linkTrackVars+",eVar9,events"
		s.linkTrackEvents="event23"
		if(s.events)s.events=s.events+",event23"
		else s.events="event23"
	}
	...
	
	{
	 ...
	 if(s.temp.toLowerCase().match('search results'))
	 s.prop36="Search"
	 
	 if(s.eVar6)s.prop36="internal campaign"
	...
	// Time Parting
   if(!s.prop17)s.prop17=s.getTimeParting('h','-7','2009'); // Set hour 
   if(!s.prop18)s.prop18=s.getTimeParting('d','-7','2009'); // Set day
   if(!s.prop19)s.prop19=s.getTimeParting('w','-7','2009'); // Set Weekend / Weekday	
	
	
if(!s.eVar9)s.eVar9 = s.crossVisitParticipation(s.eVar6,'s_evar9','90','5','>','purchase');
if(!s.eVar41)s.eVar41 = s.crossVisitParticipation(s.campaign,'s_evar41','90','5','>','purchase');
