...
var s_h_codeMarker = true;

locURL = String(window.location);

if(typeof s_accountOverride === "string" && s_accountOverride != "")
		var s_account = s_accountOverride;
else
	if((locURL.indexOf("iot.centurylink.com") == -1) && (locURL.indexOf("shop.centurylink.com") == -1) && (locURL.indexOf("www.centurylink.com") == -1) 
			&& (locURL.indexOf("q.myaccount.centurylink.com") == -1) && (locURL.indexOf("qwest.centurylink.com") == -1)
			&& (locURL.indexOf("qwest.com") == -1) && (locURL.indexOf("controlcenter.centurylink.com") == -1) && (locURL.indexOf("sbcontrolcenter.centurylink.com") == -1) 
			&& (locURL.indexOf("c.search.centurylink.com") == -1) && (locURL.indexOf("m.centurylink.com") == -1) && (locURL.indexOf("smb-resourcecenter.centurylink.com") == -1)
			&& (locURL.indexOf("digitalvault.centurylink.com") == -1) && (locURL.indexOf("espanol.centurylink.com") == -1) && (locURL.indexOf("internetcommunity.centurylink.com") == -1) 
			&& (locURL.indexOf("forums.centurylink.com") == -1) && (locURL.indexOf("centurylinkwifilocator.com") == -1) && (locURL.indexOf("qwestwifilocator.com") == -1) 
			&& (locURL.indexOf("ebill.centurylink.com") == -1) && (locURL.indexOf("promotions.centurylink.com") == -1) && (locURL.indexOf("qhelp.com") == -1) 
			&& (locURL.indexOf("ctlhelp.com") == -1) && (locURL.indexOf("reviews.centurylink.com") == -1) && (locURL.indexOf("centurylinkzone.com") == -1) 
			&& (locURL.indexOf("sorry.centurylink.com") == -1) && (locURL.indexOf("hdinternet.com") == -1) && (locURL.indexOf("news.centurylink.com") == -1))
		var s_account = "devqwestfull,qwestsmbdev";
	else
		var s_account = "qwestfull,qwestsmbprod";

var s=s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. prod fix */
...
function trackClick(qLinkName)
{
	if(!(typeof s_account === "undefined"))
	{
		...
		s.eVar6=qLinkName;
		s.tl(true,'o',qLinkName); // Kalyani change: removed if and added a new line
		if(s.templtv) s.linkTrackVars=s.templtv;
		if(s.templte)s.linkTrackEvents=s.templte;
	}
}

function trackBannerClick(qLinkName)
{
	if(!(typeof s_account === "undefined"))
	{
		var s=s_gi(s_account);
...
function trackAJAXimpressions(productString)
{
	if(!(typeof s_account === "undefined"))
	{
		var s=s_gi(s_account);
		...
		s.tl(true,'o',s.pagename + 'AJAXImp'); // Kalyani change: removed if and added a new line
		if(s.templtv) s.linkTrackVars=s.templtv;
		if(s.templte) s.linkTrackEvents=s.templte;
	}
}
function trackClickWithImpressions(qLinkName)
{
	if(!(typeof s_account === "undefined"))
	{
		...
		s.templte=s.linkTrackEvents;
		s.manageVars("clearVars");//  Kalyani change: New line added
		s.linkTrackVars='eVar52,products,events';
		s.linkTrackEvents='event23';
		s.eVar52=qLinkName;
...
/* function to reset all omniture variables if needed */
...
s.linkInternalFilters="javascript:,interprise,uswest,lci,ridethelight,cybersolutions,northwesternbell,litel,qwestinteractive.com,highway,qintra.com,qwest.connectutilities.com,interact.kefta.com,qwest.groovemobile.net,qwest.com,qwestapps.com,techzillabyqwest.com,qwestaffiliateoffers.com,qwestcampaigns.com,qwestbestoffers.com,insideconnection.com,incredibleinternet.com,qwestwifilocator.com,centurylink.com,qwestwifilocator.com,qhelp.com";
...
/*  getQueryParam() customization:
	...
	*/
	...
	if(!s.eVar52)s.eVar52=s.getQueryParam('iq_id');
	s.eVar52=s.getValOnce(s.eVar52,'s_eVar52',0);
	
	if(s.eVar52)s.events=s.apl(s.events,'event23',',',1);
	
	//link Tracking	 
	if(!s.eVar6)s.eVar6=s.getQueryParam('ln_id');
	
	//Internal campiagn stacking 
	s.eVar42 = s.crossVisitParticipation(s.eVar52,'s_evar52','90','5','>','purchase');
	...
	
	{
	 ...
	 if(s.temp.toLowerCase().match('search_results')) s.prop36="Search";
	 if(s.eVar52)s.prop36="internal campaign";	
	 if(s.campaign)s.prop36="external campaign";
	}
	
	if(!s.eVar44)s.eVar44= s.getNewRepeat();
	
	var currentDate = new Date();// Kalyani change:  added a new line
	var year = currentDate.getFullYear(); // Kalyani change: removed if and added a new line
	// Time Parting
    ...
    if(!s.eVar11)s.eVar11=s.getPreviousValue(s.pageName,'eVar11','');
...
// Kalyani change: added new code for BMG
var s_url=s.downloadLinkHandler();
if(s_url)
	{
		
		
 		s.pageName='File Downloaded:'+s_url;
	    
	    s.lnk=s.eo=0; // turn link click into page view
	}
...
s.visitorMigrationKey="4F0B2F28";
