...
function($) {
	...
	$.centurycore = {
		...
		redirectSuccessfulAuth : function(domain, modalType, redirId, customerType) {
			...
			var useJumpPage = '';
			var jumpTarget  = '';
			//console.log(domain +','+ modalType +','+ redirId +','+ customerType);
		...
		modals: {
			...
			determineCompanyByZipCode: function(zip){
				//console.log('determining zip code');
				getProfileFromZip(zip,'Y',function(data){
					console.log(data);
					qm.latestResponse = data;
					console.log(qm.latestResponse);
			...
			zam:{
				...
				initialize:function(options){
					...
					$('#zam_initial-submit').live('click',function(){$('#zam-form').submit()});
			 ...
			 */			
			 ctam:{
				...
				authenticateNewCustomer:function(){
					...
					getProfileFromAddress(street, unit, unitNumber, city, state, zip, rememberMe, function(data){
						...
						if(data.addressRequestValid) {
							...
							subcookiejar.bake('customerType',{customerType:'NC'},0,"/","centurylink.com");
				...
				authenticateExistingCustomer:function(){
					q.clearErrors();
					//console.log('authing existing');
					var radioVal=$('input[name="ec_login_type"]:checked').val();
						if(radioVal=='myaccount'){
							//console.log('myaccount');
							qmc.authExistingByMyAccount();
						}else{
							//console.log('phone or account');
				...
				authExistingByPhoneOrAccount:function(){
					...
					getProfileFromPhoneOrAccount(entry,rememberMe,function(data){
						...
						if(data.errorCode =="") {
							subcookiejar.crumble('customerType');
							subcookiejar.bake('customerType',{customerType:'EC'},0,"/","centurylink.com");
				...
				authExistingByMyAccount:function(){
					...
					if(typeof dataSource.shoppingDomain != 'undefined') {
						subcookiejar.crumble('customerType');
							subcookiejar.bake('customerType',{customerType:'EC'});
				...
				preAuthByZip:function(){
					...
					if(zip.length > 0 && zip.length < 5) {
						...
						getProfileFromZip(zip,'Y',function(data){
							qm.latestResponse = data;
							subcookiejar.crumble('customerType');
							subcookiejar.bake('customerType',{customerType:'NC'});
							if(data.shoppingDomain != '') {
								s.clearVars();
								s.prop3=s.eVar24='Existing Customer';
								s.prop51=eVar51="locator|ctam"; // Registry Method
				...
				showMyAccountAuth:function(domainOverride) {
					...
					s.events="event45";
					s.t();
					if(dataSource.zipCode == null || typeof dataSource.shoppingDomain == 'undefined') {
						$('#ctam_ec-zip').show();
						$('#ctam_ec-zip-on-file').hide();
						...
						console.log(dataSource);
						if(typeof(dataSource.domainOverlap) != 'undefined' && dataSource.domainOverlap == "Y") {
		...
		*/
		popups:{
			...
			initialize:function(options){
				$('a.popup-trigger,area.popup-trigger').each(function(){
						...
						$(this).click(function(){window.open($(this).attr('href'),'popup','scrollbars=' + scroll + ',height=' + height + ',width='+width);
						return false;
					});
		...
		analytics:{
			initialize:function(){
				...
				$('a[clicktrack],area[clicktrack]').each(function(){$(this).click(qa.trackAction);});
			...
			trackAction:function (qLinkName,withImpressions) {
				...
				if(!(typeof s_account === "undefined"))
				{
					...
					s.clearVars() // Kalyani change: New line added
					if(withImpressions) {
		...
		bannerRotator:{
			...
			initialize: function(options){
				...
				playButton.css('background-image','url(http://www.qwest.com/Homepage/images/btn_play.gif)').css('height','25px').css('width','20px');
			...
			gotoSlide:function(){
				$('#banner-toggle').css('background-image','url(http://www.qwest.com/Homepage/images/btn_pause.gif)');
				br.userPaused = true
				br.pause();
			...
			toggleRotatorState:function(){
				...
				if(br.playerState) {
					$('#banner-toggle').css('background-image','url(http://www.qwest.com/Homepage/images/btn_play.gif)');
					br.userPaused = false;
					br.play();
				} else {
					$('#banner-toggle').css('background-image','url(http://www.qwest.com/Homepage/images/btn_pause.gif)');
