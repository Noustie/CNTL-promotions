...
function getAccount(sAccount,custClass){
	if ( !!self.location.href.match("centurylink.com") && !self.location.href.match("staging") ){
		return "clinksocialmedia";
	}
	return "clinksocialmediadev";
}
function getChannel(sChannel) {
	return "";
}
function getPageName(hostName,pageName,pageNameExt) {
	return "ctl|iptv|Facebook";
}
...
s.linkInternalFilters="javascript:,centurylink.,centurytel.,embarqmail.,synacor.,embarq.,speedpay.,mspcare.bcgi.,embarqnow.,centurylink-business,seeprismtv.com"
...
*/
if ( !!window.page_tracking ) {
	$.extend( true, s, page_tracking );
}
