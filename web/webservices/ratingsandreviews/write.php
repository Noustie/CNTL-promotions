...
class RatingsAndReviews {
        ...
        foreach ($results as $key => $dataobj) {
            ...
            if ( !!$reviewHTML ) {
                $this->ReviewsMap["reviews"][] = array( 
                    "fullpath" => "/assets/feeds/third-party/bazaarVoice/" . $this->OutputRoot . "review".substr("000$key",-3).".html",
    ...
    function outputFetchList () {
        ...
        foreach ($this->AllFilesOutput as $key => $value) {
            echo "http://staging.promotions.centurylink.com/webservices/ratingsandreviews/".str_replace("./", "", $value)."\n";
        }
    }
