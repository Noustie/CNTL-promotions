...
/*************** Custom OMNITURE Processing Functions ***************/

var trimSlash=function(str){str=(str.charAt(0)=="/")?str.substr(1):str;str=(str.charAt(0)=="/")?str.substr(1):str;str=(str.charAt(str.length-1)=="/")?str.substring(0,str.length-1):str;return(str)}
var getCookie=function(c){var dc=document.cookie;return (dc.indexOf(c+'=')!=-1)?dc.split(c+'=')[1].split(';')[0]:""}
var pPath = trimSlash(location.pathname.split(";")[0]),custClass=custClass,sAccount=sAccount,sChannel=sChannel,hostName=hostName,pageName=pageName,pageNameExt=pageNameExt;

function getAccount(sAccount,custClass){
	var xTnTCookie=getCookie("xTnT");
	if(xTnTCookie=="true"||!location.host.match(/www.centurytel.com|www.ctlfiber.com|www.fiberplus.centurylink.com|www.centurylink.com|promotions.centurylink.com|business.centurytel.com|centurylink-business.com|eam.centurylink.com|sorry.centurylink.com|qwest.centurylink.com|shop.centurylink.com|espanol.centurylink.com|storelocator.centurylinkapps.com|internetcommunity.centurylink.com|healthcare.centurylink.com|faq.centurylink.com/))return "clinkdev"; 
	if(typeof(sAccount)!='undefined')return sAccount;
	if(typeof(custClass)!='undefined'&&custClass!=''){
		if(custClass=="business")return "clinkbusiness,clinkcompany,clinkCTLdomain";
		if(custClass=="consumer")return "clinkconsumer,clinkcompany,clinkCTLdomain";
	}
	if(pPath==""||  
	pPath=="index.jsp"||
	pPath=="Pages/Homepage/homepageLanding.jsp"||
	pPath=="Pages/Identification/maIdentification.jsp"||
	pPath.indexOf("MyAccount")!=-1|| //no custClass identifier
	//pPath=="Pages/MyAccount/loginForm.jsp"||
	//pPath=="MyAccount/PasswordReset/passwordReset.html"||
	//pPath=="Pages/MyAccount/showAccountNumber.jsp"||
	//pPath=="Pages/MyAccount/enrollment.html"||
	//pPath=="Pages/MyAccount/overview.jsp"||
	pPath=="Pages/SiteMap"||
	pPath=="storeLocator.html"||
	pPath=="rewardsLanding.html"||
	pPath=="Pages/Search"||	
	pPath=="Pages/Search/searchTips.jsp"||
	pPath=="Pages/Errors/404Error.jsp"||
	pPath=="pages/errors/404error.jsp"||
	pPath.indexOf("eam/")!=-1||	
	pPath.indexOf("upport")!=-1){	
		return "clinkcommon,clinkcompany,clinkCTLdomain";
	}else if(pPath=="Pages/Identification/wholesaleIdentification.jsp"||pPath.indexOf("holesale")!=-1){
		return "clinkwholesale,clinkcompany,clinkCTLdomain";
	}else if(pPath.indexOf("AboutUs")!=-1){
		return "clinkcorporate,clinkcompany,clinkCTLdomain";
	}else if(location.host=="centurylink-business.com "||pPath.toLowerCase().indexOf("biz")!=-1||pPath.indexOf("usiness")!=-1){
		return "clinkbusiness,clinkcompany,clinkCTLdomain";
	}else{
		return "clinkconsumer,clinkcompany,clinkCTLdomain";
	}
}
function getChannel(sChannel) {
	if(typeof(sChannel)!='undefined')return sChannel;
	if(pPath==""||
	pPath=="index.jsp"||
	pPath=="Pages/Homepage/homepageLanding.jsp"||
	pPath=="shop/"||
	pPath=="Pages/Personal"||	
	pPath=="business"||
	pPath=="Pages/SiteMap"||	
	pPath=="Pages/Search"||	
	pPath=="Pages/Search/searchTips.jsp"||
	pPath=="Pages/Errors/404Error.jsp"||
	pPath=="pages/errors/404error.jsp"||
	pPath.indexOf("holesale")!=-1||
	pPath.indexOf("AboutUs")!=-1){
		return "";
	}else if(pPath=="Pages/Identification/maIdentification.jsp"||pPath.indexOf("eam/")!=-1||pPath.indexOf("MyAccount")!=-1){
		return "MyAccount";
	}else if(pPath.indexOf("Discover")!=-1){
		return "Discover";
	}else if(pPath.indexOf("Support")!=-1||pPath.indexOf("CustomerService")!=-1||pPath=="rewardsLanding.html"||location.host=="faq.centurylink.com"){
		return "Support";
	}else{
		return "Shopping";
	}
}
function getPageName(hostName,pageName,pageNameExt) {
	if(typeof(pageName)=='undefined'){
		var pageName=pPath.replace(/index.jsp|Pages\/Homepage\/homepageLanding.jsp/gi,'')
		if(pageName.indexOf('.flow')!=-1){
			var shopStep=location.search.split('execution=')[1]||'';
			shopStep=shopStep.split('&')[0].split(';')[0].charAt(shopStep.length-1);
			if(shopStep.length>0)pageName+="_v"+shopStep;
		}
		if(pPath=="removeProduct.html"){
			var prodKey=location.search.split('productKey=')[1]||'';
			prodKey=prodKey.split('&')[0].split(';')[0];
			if(prodKey.length>0)pageName+="_KEY"+prodKey;
		}
		if(pPath=="checkoutExistingCustomer.html"&&sEvents=="purchase")pageName+="_COMPLETE";
	}
	if(typeof(pageNameExt)!='undefined')pageName=pageName+pageNameExt;
	return ((typeof(hostName)!='undefined')?hostName:'CTL')+':'+((pageName==''||pageName=='/')?'Home':pageName);
}
var s_account=getAccount(sAccount,custClass);
...
s.linkInternalFilters="javascript:,centurylink.,centurytel.,embarqmail.,synacor.,embarq.,speedpay.,mspcare.bcgi.,embarqnow.,centurylink-business"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"
s.visitorNamespace="centurylink"
s.trackingServer="metrics.centurylink.com"
s.trackingServerSecure="smetrics.centurylink.com"
/* Plugin Config */
s.usePlugins=true
...
/***********************global functions**********************************/
	...
	if((window.s.linkTrackVars === undefined) || window.s.linkTrackVars == "None")s.linkTrackVars = "prop17,prop18,prop19";
		else if(!(s.linkTrackVars.indexOf('prop17,prop18,prop19')>-1))s.linkTrackVars = s.linkTrackVars+",prop17,prop18,prop19";
	
	/* Add calls to plugins here */		
	// External Campaigns
/*  getQueryParam() customization:
	  tid= Banner Campaign ID
	  rid= Redirect / Email tracking ID
	  pid= Paid Search ID
	  mid= DFA Embarq Tracking Code
	  kid= Google Affiliate Tracking Code
	  jid= YesMail/CheetahMail Embarq Tracking Code
	  lid= YesMail MessageID or MMID
	  sid= DFA Genesis Integration ID
	  wid= YesMail Visitor ID
	  bid= CheetahMail Subscriber ID
	  fid= CheetahMail Message ID
	  refCode=Legacy qwest campiagn tracking code 
	*/

	//Capture External Campaign Code
	s.campaign=s.getQueryParam('rid,pid,jid,mid,gid,kid,nid,sc_cid,s_cid,refCode',':');

	s.campaign=s.getValOnce(s.campaign,'s_campaign',0);
	
	//Campiagn stacking
	s.eVar22 = s.crossVisitParticipation(s.campaign,'s_cpm','30','5','>','purchase');	
	...
	{
	   if(s.campaign)
	   s.prop2= s.pageName+" | Campaign-"+s.campaign	    
	   else if(camp)
            s.prop2=s.pageName;
	}
	...
	if(!s.eVar38)s.eVar38=s.getQueryParam('tid')
	s.eVar38=s.getValOnce(s.eVar38,'s_eVar38',0);
	
	if(s.eVar38)s.events=s.apl(s.events,'event23',',',1)
	
	
	//link Tracking	 
	if(!s.eVar30)s.eVar30=s.getQueryParam('ln_id')
	
	//Internal campiagn stacking 
	s.eVar42 = s.crossVisitParticipation(s.eVar38,'s_evar38','90','5','>','purchase');
	
	// A/B testing 	
	// Commenting  A/B testing as per the WCP:130225-6
	/*if(!s.prop7)
	{
	 if(s.pageName)s.prop7=s.pageName
	}
	else
	{
	...
	*/
	if(!s.prop36)// finding methods
	
	{
	 s.prop36="Browsing"
	 s.temp=s.getPreviousValue(s.pageName,'gpv_ptemp','');
	 if(s.temp.toLowerCase().match('search')) s.prop36="search"
	 if(s.eVar38)s.prop36="internal campaign"	
	 if(s.campaign)s.prop36="external campaign"
	}
	
	if(!s.eVar44)s.eVar44= s.getNewRepeat();
	
	var currentDate = new Date();
	var year = currentDate.getFullYear(); 
	// Time Parting
    if(!s.prop17)s.prop17=s.getTimeParting('h','-7',year); // Set hour  		// Kalyani change: removed the hard coded year and added a variable
    if(!s.prop18)s.prop18=s.getTimeParting('d','-7',year); // Set day   		// Kalyani change: removed the hard coded year and added a variable
    if(!s.prop19)s.prop19=s.getTimeParting('w','-7',year); // Set Weekend / Weekday	// Kalyani change: removed the hard coded year and added a variable
    
    if(!s.eVar61)s.eVar61=s.getPreviousValue(s.pageName,'eVar61','')
    
   // Testid pathing
    if(!s.eVar63)
	{
	 if(s.pageName)s.prop40=s.pageName
	}
	else
	{
	s.prop40=s.eVar63+" | " +s.pageName
	}
...
/*****************************CTL CUSTOM CODE*********************************/
	...
	s.prop1=s.getQueryParam('rid');  //s.prop1 which replaced s.referrer
	s.eVar19=s.prop1;  //associates campaigns to sales
	...
	/* Set de-duped onsite search event */
		   var t_search=s.getValOnce(s.eVar62,'ev62',0)
		   if(t_search)
	...
	//YesMail RecipientID
	s.eVar33 = s.getQueryParam('wid');

	//YesMail Integration Report for MMID
	s.eVar35 = s.getQueryParam('lid');

	//CheetahMail SubscriberID
	s.eVar37 = s.getQueryParam('fid');

	//CheetahMail Integration Report for MessageID
	s.eVar36 = s.getQueryParam('bid');
...
// Kalyani change: removed the hard coded year and added a variable
/*
...
//Kalyani change: download functions added 
...
/************* HERE ARE THE OMNITURE PAGE CODE VARIABLES ************/
if (typeof sDisplayErr == "undefined"){ var sDisplayErr=""; }
if (typeof sMarket == "undefined"){ var sMarket=""; }
...
//Fix for defects 6665 and 6664  and now defect 7748

if(typeof subcookiejar != 'undefined'  && subcookiejar) {                            
                /* WCP 120326-4 : Prod issue for channel */
             if(typeof eBiz_channel != 'undefined' && eBiz_channel) {
                    if(eBiz_channel == 'rsd' && subcookiejar.fetch('profile_cookie')) {
                       var cookieData = subcookiejar.fetch('profile_cookie');
                       if((cookieData.domainOverlap == 'N' && cookieData.shoppingDomain == 'Q') ||
						  (cookieData.domainOverlap == 'Y' && cookieData.shoppingDomainOverride == 'Q')) {
                                s.channel = 'RSD:Shopping';
						 } else {
...
/* end fix for defects 6664 and 6665*/
...
if(typeof eBiz_evar16 != 'undefined' && eBiz_evar16)s.eVar16=eBiz_evar16;
...
if(typeof ttPrismDMAName != 'undefined' && ttPrismDMAName)s.eVar63=ttPrismDMAName;

if(typeof sProp5 != 'undefined' && sProp5)s.prop5=sProp5;
if(typeof sProp9 != 'undefined' && sProp9)s.prop9=sProp9;
if(typeof sProp16 != 'undefined' && sProp16)s.prop16=sProp16;
if(typeof sEvar14 != 'undefined' && sEvar14)s.eVar14=sEvar14;
if(typeof sEvar32 != 'undefined' && sEvar32)s.eVar32=sEvar32;
if(typeof sEvar40 != 'undefined' && sEvar40)s.eVar40=sEvar40;
if(typeof sEvar45 != 'undefined' && sEvar45)s.eVar45=sEvar45;

if(!s.pageName)s.pageName=getPageName(hostName,pageName,pageNameExt);
if(!s.channel)s.channel=getChannel(sChannel);



//Below code is not required as we are assigning the s.channel=eBiz_channel in above subcookiejar condition
/*if(s.pageName.indexOf("ctl")!=-1) {

	if(s.pageName.indexOf("rsd")!= -1  &&  s.pageName.indexOf("shop") == -1 )
		{
			s.channel ="RSD:Shopping";
		}
		else 
...
if(!s.eVar1)s.eVar1=accountEvents;
if(!s.eVar2)s.eVar2=serviceEvents;
if(!s.eVar3)s.eVar3=pCategory;
if(!s.eVar4)s.eVar4=unisysEvent;
if(!s.eVar7)s.eVar7=orderType;
if(!s.eVar8)s.eVar8=zipAvail;
if(!s.eVar9)s.eVar9=phoneAvail;
if(!s.eVar10)s.eVar10=addressAvail;
if(!s.eVar11)s.eVar11=respMsg;
if(!s.eVar12)s.eVar12=availType;
s.events=sEvents;
if(!s.eVar28)s.eVar28=sDisplayErr;
if(!s.eVar39)s.eVar39=sMarket;
if(!s.eVar29)s.prop29=sMarket;
if(!s.products)s.products= sProducts; 
if(!s.purchaseID)s.purchaseID= orderNum;
if(typeof sPageURL != 'undefined') s.pageURL = sPageURL;
